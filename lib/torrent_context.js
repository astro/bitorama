...
var TrackerGroup = require('bittorrent-tracker-client').TrackerGroup;
...
var STREAM_READAHEAD_TIME = 10.0;
...
TorrentContext.prototype._canRequest = function(wire) {
    /* Pieces per second */
    var rps = wire.downloadSpeed() / DataDownload.chunkSize;
    var minReqs = Math.max(2, Math.ceil(0.5 * rps));
    var maxReqs = Math.max(minReqs + 1, Math.ceil(1 * rps));
    // console.log("reqs:", minReqs, "..", maxReqs, "x", DataDownload.chunkSize, "@", wire.downloadSpeed());
    ...
    while(needMore) {
        ...
        var piecesThreshold = Math.max(
            1, 
            Math.ceil(STREAM_READAHEAD_TIME * this.swarm.downloadSpeed() / this.pieceLength)
        );
        needMore = wire.requests.length < maxReqs && this.download.pieces.length < piecesThreshold;
        if (needMore) {
...
TorrentContext.prototype.streamFile = function(fileName, offset, length) {
    ...
    if (file) {
        ...
        console.log("length", length, "file.length", file.length, "offset", offset);
        if (!isNaN(length)) {
        ...
        stream.fileLength = file.length;
        stream.on('priorize', function(index) {
            ...
            this.download.priorizePieces(indexes);
        }.bind(this));
        return stream;
    } else {
        return null;
